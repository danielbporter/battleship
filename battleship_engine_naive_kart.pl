...
initial_configuration(player_dp, InitalConfiguration) :- initial_configuration_naive_player2(InitialConfiguration).
...
next_moves_naive_dp(_, [], _, _, _, next_moves([position(a, 1)])) :- write(“first move made”).
next_moves_naive_dp(initial_configuration(Ships), own_history([position(LastR, LastC) | OwnHistory]),
                    opponent_history(OpponentHistory), own_board(OwnBoard), opponent_board(OpponentBoard),
                    next_moves([position(Rmove, Cmove)])) :-
    next_sequential_position_dp(position(LastR, LastC), position(NextR, NextC)).
...
next_sequential_position_dp(position(R1, C1), position(R2, C2)) :- 
    increment(R1,R2), increment(C1,C2).
